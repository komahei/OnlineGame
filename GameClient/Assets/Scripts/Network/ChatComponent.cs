...
namespace GameClient
{
	public class ChatComponent : MonoBehaviour, IChatHubReceiver
	{
		...
		public TextMeshProUGUI JoinnOrLeaveButtonText;
		...
		private void InitializeUi()
		{
			...
			this.Input.placeholder.GetComponent<TMP_Text>().text = "Please enter your name";
			this.JoinnOrLeaveButtonText.text = "Enter the room";
		...
		private async void RegisterDisconnectEvent(IChatHub streamingClient)
		{
			...
			{
				// try-to-reconnect? logging event? closes? etc...
		...
		public async void DisconnectServer()
		{
			...
			if (this.isJoin) this.JoinOrLeave();

			await this.streamingClient.DisposeAsync();
		}

		public async void ReconnectInitializeServer()
		{
		...
		public async void JoinOrLeave()
		{
			...
			{
				var request = new JoinRequest { RoomName = "SampleName", UserName = this.Input.text };
				await this.streamingClient.JoinAsync(request);

				this.isJoin = true;
				this.SendMessageButton.interactable = true;
				this.JoinnOrLeaveButtonText.text = "Leave the room";
				this.Input.text = string.Empty;
				this.Input.placeholder.GetComponent<TMP_Text>().text = "Please enter a comment.";
				this.ExceptionButton.interactable = true;
			}
		}
